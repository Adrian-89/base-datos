...
SELECT id_producto, SUM(cantidad) AS sum_total FROM Pedidos GROUP BY id_producto;
/**
┌─────────────┬───────────┐
│ id_producto │ sum_total │
├─────────────┼───────────┤
│ 1           │ 2         │
│ 2           │ 1         │
│ 3           │ 3         │
...
SELECT id_cliente, COUNT(id_pedido) AS total_pedidos FROM Pedidos GROUP BY id_cliente HAVING total_pedidos > 1;
-- No daria ningun resultado ya que no hay parametros que coincidan 
...
SELECT MIN(fecha_pedido) AS primera_fecha FROM Pedidos;
/**
┌───────────────┐
│ primera_fecha │
├───────────────┤
...
SELECT nombre,max(precio) AS max_price FROM productos;
/**
┌────────┬───────────┐
│ nombre │ max_price │
├────────┼───────────┤
...
SELECT MAX(fecha_pedido) AS last_order FROM Pedidos;
/**
┌─────────────────────┐
│     last_order      │
├─────────────────────┤
...
SELECT MIN(precio) AS min_price FROM Productos;
/**
┌───────────┐
│ min_price │
├───────────┤
...
SELECT MAX(fecha_pedido) AS las_date FROM Pedidos WHERE id_cliente = 3;
/**
┌─────────────────────┐
│      last_date      │
├─────────────────────┤
